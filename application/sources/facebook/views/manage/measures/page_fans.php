					...
					if( FBObj ) {
						...
						$.getJSON('<?= base_url() ?>services/source/<?= $source->getId( ) ?>/test/measure/<?= $measure['source_measure'] ?>', data, function ( response ) {
							...
							if( response.error && response.error.code ) {
								...
								$fbRequest.removeClass('orange').addClass('blue').html('<i class="facebook icon"></i><div class="content"><div class="header">PAGE FANS: ' + ( response.data[0].fans + '' ).replace(/\d(?=(\d{3})+$)/g, '$&.') + '</div><p>Today\'s total at 0 hours.</p></div>' );
							}
						});
					}
				}
