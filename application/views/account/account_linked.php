...
<head>
	<?= $this->load->view('admin/head', array('title' => lang('linked_page_name')) ) ?>
</head>
<body>
<?= $this->load->view('admin/header') ?>
<div class="container content">
	<?= $this->load->view('admin/menu', array('current' => 'account_linked') ) ?>
...
<?
	...
	if ( !empty($errors) ) {
		...
		$this->load->view('admin/message', array('type' => 'success','class' => 'info','content' => $this->session->flashdata('linked_info')) );
	}
	else {
		$this->load->view('admin/message', array('class' => 'warning sign','content' => lang('linked_page_satement')) );
	}
?>
            <h3><?php echo lang('linked_currently_linked_accounts'); ?></h3>
<?
	if( $num_of_linked_accounts == 0 ) {
		$this->load->view( 'admin/message', array( 'type' => 'warning', 'class'   => 'warning sign', 'content' => lang( 'linked_no_linked_accounts' ) ) );
	}
	else {
?>
